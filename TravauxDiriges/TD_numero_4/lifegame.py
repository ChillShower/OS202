...
class App:
    ...
    def draw(self):
        nb_lig_per_process=self.grid.dimensions[0]//comm_size
        a=rank*nb_lig_per_process #rang du premier élément
        if rank!=comm_size-1:
            b= a+nb_lig_per_process #dernière colonne
        else:
            b=self.grid.dimensions[0]
        tab=[]
        for i in range(a,b):
            [self.screen.fill(self.compute_color(i,j),self.compute_rectangle(i,j)) for j in range(self.grid.dimensions[1])]
